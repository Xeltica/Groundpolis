...
export default class AutoComplete extends Vue {
	@Prop() private type: 'user' | 'emoji' | 'hashtag';
	@Prop() private q: string;
	@Prop() private textarea: HTMLTextAreaElement;
	@Prop() private complete;
	@Prop() private close: () => void;
	@Prop() private x: number;
	@Prop() private y: number;

	private getStaticImageUrl = getStaticImageUrl;
	private fetching = true;
	private users = [] as User[];
	private hashtags = [] as string[];
	private emojis = [] as EmojiDef[];
	private select = -1;
	private emojilib;
	private emojiDb = [] as EmojiDef[];
	...
	public updated() {
		...
		if (this.x + el.offsetWidth > window.innerWidth) {
			el.style.left = (window.innerWidth - el.offsetWidth) + 'px';
		} else {
			el.style.left = this.x + 'px';
		}

		if (this.y + el.offsetHeight > window.innerHeight) {
			el.style.top = (this.y - el.offsetHeight) + 'px';
			el.style.marginTop = '0';
		} else {
			el.style.top = this.y + 'px';
			el.style.marginTop = 'calc(1em + 8px)';
		}
		//#endregion
	}
